...
const Trending = (props) => {
    ...
    return (
        <main className="trending">
            ...
            {data.length === 0 ? <p className="not-found">No Trending Found</p> :
                <div className="boxes">
                    <Swiper spaceBetween={16} slidesPerView={"auto"} initial="initial" animate="animate" breakpoints={{ 701: { spaceBetween: 30 }, 1201: { spaceBetween: 40 } }}>
                        {data.map((ele, index) => {
                    ...
                    {counter === 0 ? <p className="not-found">No Trending Found</p> : ''}
                </div>}
        </main>
    )
}
